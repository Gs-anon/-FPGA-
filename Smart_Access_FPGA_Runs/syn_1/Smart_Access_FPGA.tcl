...
set rc [catch {
  open_project {Smart_Access_FPGA.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {smart_access_uart_board_top_multi}
  export_db {Smart_Access_FPGA_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../constraint/smart_access.adc
  optimize_rtl
  report_area -file Smart_Access_FPGA_rtl.area
  export_db {Smart_Access_FPGA_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea Smart_Access_FPGA_gate.area
  legalize_phy_inst
  export_db {Smart_Access_FPGA_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
